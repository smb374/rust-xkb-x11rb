...
use crate::{Keycode, LayoutMask, ModMask};
use ffi::*;
use x11rb::xcb_ffi::XCBConnection as Connection;
...
#[inline]
pub fn device(connection: &Connection) -> Result<i32, ()> {
    unsafe {
        match xkb_x11_get_core_keyboard_device_id(connection.get_raw_xcb_connection() as *mut _) {
            -1 => Err(()),
            id => Ok(id),
        }
    }
...
pub fn keymap(
    ...
    unsafe {
        ...
        .as_mut()
        .map(|ptr| Keymap::from_ptr(ptr))
        .ok_or(())
    }
}

#[inline]
pub fn state(connection: &Connection, device: i32, keymap: &Keymap) -> Result<State, ()> {
    unsafe {
