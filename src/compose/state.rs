...
use crate::compose::{Result, Status, Table};
use crate::Keysym;
...
impl State {
    ...
    pub fn table(&self) -> Table {
        unsafe { Table::from_ptr(xkb_compose_state_get_compose_table(self.0)) }
    }

    #[inline]
    pub fn feed<T: Into<Keysym>>(&mut self, sym: T) -> Result {
        unsafe { mem::transmute(xkb_compose_state_feed(self.0, sym.into().into())) }
    }
    ...
    pub fn status(&self) -> Status {
        unsafe { mem::transmute(xkb_compose_state_get_status(self.0)) }
    }
...
impl Clone for State {
    #[inline]
    fn clone(&self) -> Self {
        unsafe { State(xkb_compose_state_ref(self.0)) }
    }
}
...
impl Drop for State {
    #[inline]
    fn drop(&mut self) {
        unsafe { xkb_compose_state_unref(self.0) }
    }
}
