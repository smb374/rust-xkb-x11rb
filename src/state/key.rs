...
use crate::{Keycode, Keysym, State};
...
impl<'a> Key<'a> {
    ...
    pub fn consumed(&self, mode: Consumed) -> ModMask {
        unsafe {
            xkb_state_key_get_consumed_mods2(self.0.as_ptr(), self.1.into(), mem::transmute(mode))
                .into()
        }
    }
