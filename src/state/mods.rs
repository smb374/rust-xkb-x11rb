    ...
    pub fn active<P: Into<Parameter>>(&self, parameter: P, component: Components) -> bool {
        unsafe {
            match parameter.into() {
                Parameter::Name(name) => {
                    xkb_state_mod_name_is_active(self.0.as_ptr(), name.as_ptr(), component.bits())
                        != 0
                }

                Parameter::Index(index) => {
                    xkb_state_mod_index_is_active(self.0.as_ptr(), index.into(), component.bits())
                        != 0
                }
            }
        }
