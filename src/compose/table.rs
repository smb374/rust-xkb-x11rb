...
impl Table {
    ...
    pub fn new<L: AsRef<str>>(
        ...
        unsafe {
            ...
            xkb_compose_table_new_from_locale(context.as_ptr(), locale.as_ptr(), flags.bits())
                .as_mut()
                .map(|ptr| Table(ptr))
                .ok_or(())
        }
    ...
    #[inline]
    pub fn from_buffer<L: AsRef<str>, B: AsRef<[u8]>>(
        context: &Context,
        locale: L,
        buffer: B,
        format: Format,
    ...
    pub fn state(&self, flags: state::Flags) -> State {
        unsafe { State::from_ptr(xkb_compose_state_new(self.0, flags.bits())) }
    }
}
...
impl Clone for Table {
    #[inline]
    fn clone(&self) -> Self {
        unsafe { Table(xkb_compose_table_ref(self.0)) }
    }
}
...
impl Drop for Table {
    #[inline]
    fn drop(&mut self) {
        unsafe { xkb_compose_table_unref(self.0) }
    }
}
